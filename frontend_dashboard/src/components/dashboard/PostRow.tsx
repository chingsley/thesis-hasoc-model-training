...
const STATUS_FLOW: TriageStatus[] = ['new', 'reviewed', 'reported']
    ...
    <div className="flex items-center gap-4 p-4 border-b border-border last:border-0 hover:bg-muted/30 transition-colors">
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {post.hate_target_category && (
            <Badge variant="outline" className="text-xs capitalize">{post.hate_target_category}</Badge>
          )}
      ...
      <div className="flex items-center gap-3 shrink-0">
        ...
        <Select
          value={post.triage_status}
          onValueChange={(v) => v && statusMutation.mutate({ postId: post.id, status: v as TriageStatus })}
        >
